...
#define ARDUINO 0x12
#define K_DATA_HIMAX 6

// #define FRACTIONAL_BITS 5

const int8_t kFractionalBits = 5;

int8_t data_buf[K_DATA_HIMAX];
...
inline int8_t float_to_fixed(float x) {
  return (int8_t)(round(x * (1 << kFractionalBits)));
}
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (hal_gpio_arduino_in.gpio_data == 1) {
      // TODO calculate jerk, use 16 bit for precision?
      ...
      data_buf[4] = 7;
      data_buf[5] = 87;
      hx_drv_i2cm_set_data(ARDUINO, NULL, 0, data_buf, K_DATA_HIMAX);
      hal_led_r.gpio_data = !hal_led_r.gpio_data;
      hx_drv_gpio_set(&hal_led_r);
      // for (int i = 0; i < K_DATA_HIMAX; ++i) {
      //   hx_drv_uart_print("%d ", data_buf[i]);
      // }
      // hx_drv_uart_print("\n");
    } else {
      hal_gpio_set(&hal_led_r, GPIO_PIN_RESET);
